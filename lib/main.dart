...
class MyAppState extends State<MyApp> {
    ...
    if (isAndroid) {
      ...
      var rawVoice = rawVoices.firstWhere(
        ...
        orElse: () => rawVoices.firstWhere(
          (v) => v?['locale']?.startsWith(myLocale) ?? false,
        ),
      );
      voice = rawVoice;
  ...
  List<DropdownMenuItem<Map<String, String>?>> getVoicesDropDownMenuItems(
    ...
    if (voiceItems.isEmpty) {
      ...
      for (dynamic item in voices) {
        ...
        if (!voiceItems.any(
          (element) => mapEquals(element.value, menuItem.value),
        )) {
          voiceItems.add(menuItem);
        }
      }
      voiceItems.sort((a, b) {
        return a.child.toString().toLowerCase().compareTo(
          b.child.toString().toLowerCase(),
        );
      });
    }
  ...
  Future<void> changedLanguagesDropDownItem(String? selectedLanguage) async {
    ...
    if (voiceItems.isNotEmpty) {
      var voiceItem = voiceItems.firstWhere(
        (v) => v.value?['locale'] == selectedLanguage,
      );
